    ...
    <div class="container mt-3">
        <div class="user-form col-12 col-lg-6">
            <div class="mb-4">
                ...
                <p class="text-muted mb-0">Tambahkan pengguna baru ke dalam sistem restoran.</p>
            </div>
            ...
            <form @submit.prevent="createUser">
                ...
                <div class="mb-3">
                    ...
                    <input id="password" v-model="form.password" type="password" class="form-control"
                        placeholder="Password pengguna" minlength="6" required>
                </div>
                ...
                <div class="d-flex gap-2">
                    ...
                    <RouterLink class="btn btn-outline-secondary" to="/">Batal</RouterLink>
                </div>
            </form>
        </div>
    </div>
...
export default {
    ...
    methods: {
        getRoles() {
            axios.get(`${import.meta.env.VITE_API_URL}/role`, {
                headers: {
                ...
                .catch((error) => {
                    console.error(error)
                    alert('Data role gagal dimuat')
                })
        },
        createUser() {
            this.submitting = true

            axios.post(`${import.meta.env.VITE_API_URL}/user`, this.form, {
                headers: {
                    Authorization: `Bearer ${localStorage.getItem('token')}`,
                },
            })
                ...
                .catch((error) => {
                    console.error(error)
                    alert(error.response?.data?.message || 'Pengguna gagal ditambahkan')
                })
                .finally(() => {
